    ...
    <AnimatePresence mode="wait">
      {isOpen && (
        <motion.dialog
          ...
          className={cn(
            "fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform overflow-visible rounded-lg border border-zinc-200 p-0 shadow-lg",
            "backdrop:bg-neutral-400/60",
...
function DialogClose({ className, children, disabled }: DialogCloseProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute top-3 right-3 rounded-xs opacity-70 transition-opacity",
        "hover:cursor-pointer hover:opacity-100 focus:outline-hidden",
        "text-neutral-700 disabled:pointer-events-none",
        className,
      )}
      disabled={disabled}
    >
